...
export const ucwords = ( string ) => this.words( string ).map( word => ucword( word ) ).join(' ');
...
export const unPascal = function( value, seperator = '_' ){
    return value.replace(/[A-Z]/g, m => " " + m.toLowerCase()).trim().replace(' ', seperator );
}
...
class StringUtil {
    ...
    static sprintx( string, args=[], tokens=[] ){
        ...
        for(let i=0;i<args.length;i++){
            ...
            const replacer = function(p,c){ return p.replace(`%${tokens[i]}`,c)};
    ...
    static sprintf( string, args ){
        const tokens = [...args];
        var replacer = function(p,c){ args.shift(); return p.replace(/%s/,c)};
        return tokens.reduce( replacer, string );
    }

    static sprintMake( string, args ){
        var replacer = function(p,c){return p.replace(c, '%s')};
        return args.reduce( replacer, string );
    }
